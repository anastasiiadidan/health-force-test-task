...
from aws.constants import DATABASE_URL, PATIENT_DATA_URL, PATIENT_IDENTITY_URL
...
def _retrieve_pic_from_identify_page(html_body: str) -> str:
    """
    Finds patient picture from the identity page

    Parameters
    ----------
    html_body : str
        HTML page that should be parsed on finding the picture
...
def _fetch_pic_from_database(webdriver: WebDriver, patient_pnr: str) -> str:
    """
    Fetches patient picture of the database.
    ...
    -------
    str
        Patient picture binary
    """
    payload = {"page": 1, "pageSize": 50, "FieldFilter": patient_pnr}
    result = webdriver.post(DATABASE_URL, payload=payload)
    patient_pic = result.json()["Data"][0]["NumeroAuth"]
    return patient_pic


def _check_request_accepted(
    webdriver: WebDriver, patient_pnr: str, patient_esame: str, code_prestazioni: str
):
    """
    Validates patient data in the system with selenium webdriver
    ...
    code_prestazioni : str
        The first priority patient code.
    ...
    ------
    ValueError
        When no item for patient code was found.
    """
    webdriver.get(PATIENT_IDENTITY_URL, backend="selenium")
    ...
    # This allows to try the code that we have in the list
    # and if it doesn't work try the next one. This list comes from the categories
    # to chose on the site in order to create a prior-authorization
    ...
    for current_code in codes_to_try:
        for item in webdriver.find_elements(by=By.CLASS_NAME, value="k-item"):
            if item.text == current_code:
                break
        # The 'else' part of a for loop is executed when the loop
        # completed normally (i.e., did not encounter a break statement)
        # In this case, we immediately continue with the next iteration 
        # of the outer loop, skipping the rest of this iteration
        else:
            continue
        # If we've reached this point, it means the inner loop broke (i.e., we found the item)
        # So we break the outer loop as well
        break
    # The 'else' part of the outer for loop is executed only if the loop
    # completed normally (i.e., we did not find the item with any of the codes)
    else:
        # If we've reached this point, we did not find
        # the item with any of the codes, so we raise an error
    ...
    # TODO: We are assuming for now that the prestazioni is possible,
    # we need to find PNR for which it is not.
    webdriver.click_js(webdriver.find_element(by=By.ID, value="btnVerifica"))
    # Retrieve the message
    # TODO: We still have not got any PNR for which the insurance refuses the patient.
    # So we cannot assess that it catches error messages.
    time.sleep(1)
...
def process_pnr(
    ...
    """
    Gets patient data from its PNR
    ...
    engine : Object
        Data processor
    patient_data : dict
        The disctionary with the data of single patient
    pnr : str
        The patient's unique PNR
    pnr_status_manager : Object
        Retrieves patient status from its PNR
    comments : list
        The list of comments that are going to added to the Excel report
    
    Returns
    -------
    tuple
        Patient data and comments
    """

    # Fetch the patient PNR state once.
    # This page also contains useful data for a lot of states.
    ...
    if rule_engine.Rule("pnr_status in [1,2]").evaluate(patient_data):
        ...
        if rule_engine.Rule("pnr_status == 1").evaluate(patient_data):
            _check_request_accepted(
                webdriver, pnr, patient_data["esame"], patient_data["prestazioni"]
            )
...
def process_patient(
    ...
    Try to handle as much edge cases as possible.
    But still missing data and states never seen during production.
    ...
    webdriver : WebDriver
        Selenium webdriver instance
    patient_data : dict
        The dictionary with single patient data
    response : Response
        The response instance that contains patiaent data we'll send to the hospital
    config : dict
    ...
    # Go through the rules that moat the rest of the script
    # from running as there's no sufficient data
    ...
    # Even if the patient doesn't need 2 PNR we process all the PNR
    # that we found to be sure that we are not missing something
    ...
    response.add_patient(patient_data=patient_data, comments=" / ".join(comments))
